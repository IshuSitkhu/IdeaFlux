...
const AuthorProfile = () => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ marginBottom: 6, wordBreak: "break-word" }}>
          {author.name}
        </h2>
        ...
        {showFollowers && (
          ...
          >
            {author.followers?.length === 0 ? (
              ...
              author.followers.map((follower) => (
                <p key={follower._id} style={{ marginBottom: 6, wordBreak: "break-word" }}>
                  <Link
                    to={`/user/${follower._id}`}
                    style={{ color: "#2563eb", textDecoration: "underline" }}
        ...
        {showFollowing && (
          ...
          >
            {author.following?.length === 0 ? (
              ...
              author.following.map((user) => (
                <p key={user._id} style={{ marginBottom: 6, wordBreak: "break-word" }}>
                  <Link
                    to={`/user/${user._id}`}
                    style={{ color: "#2563eb", textDecoration: "underline" }}
        ...
        {author.bio && <p style={{ marginBottom: 10, wordBreak: "break-word" }}>{author.bio}</p>}
      ...
      {/* Blogs */}
      <h3 style={{ marginBottom: 16 }}>{author.name.split(" ")[0]}'s Blogs</h3>
      {blogs.length === 0 ? (
        <p style={{ color: "#666", fontStyle: "italic" }}>No blogs published yet.</p>
      ) : (
        <div
          style={{
            display: "grid",
        ...
        >
          {blogs.map((blog) => (
            <BlogCard key={blog._id} blog={{ ...blog, author }} currentUserId={currentUser?.id} />
          ))}
        </div>
      )}
    </div>
